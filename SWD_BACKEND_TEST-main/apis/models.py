...
def validator(value):
    ...
    if value > 100 or value < 0 :
        raise ValidationError(
            _('%(value)s is greater than 100 or less than 0'),
            params={'value': value},
        )
...
class StudentSubjectsScore(models.Model):
    student = models.ForeignKey(Personnel, on_delete=models.CASCADE, null=False, blank=False,validators=[validators.validate_slug])
    subjects = models.ForeignKey(Subjects, on_delete=models.CASCADE, null=False, blank=False,validators=[validators.validate_slug])
    credit = models.IntegerField(null=False, blank=False)
    score = models.FloatField(null=False, blank=False,max_length=50, default=0, validators=[validator])
